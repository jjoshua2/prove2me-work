...
/-! All-vertex diameter transfer through simultaneous finite clipping.
Face budgets are explicit hypotheses for the FINAL cut faces. -/
...
/-- Endpoints may be new vertices. Both radial spokes and the old walk share
one finite support family, so no final face is charged more than once. -/
...
/-- All-vertex finite clipping, without a separate strict-feasibility assumption.
The face budgets refer to the final intersection, not intermediate cuts. -/
