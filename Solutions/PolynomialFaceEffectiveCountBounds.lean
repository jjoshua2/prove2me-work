...
/-- In a balanced separated instance, every source neighbor shares at least
one target row, so its target-common face has at most `2d-1` effective rows. -/
theorem balanced_neighbor_effectiveCount_le_two_d_sub_one
    (d : ℕ)
  ...
  have hone : 1 ≤ (commonSourceRows a b v z).card :=
