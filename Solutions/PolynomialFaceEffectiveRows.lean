...
/-- Delete exactly the rows whose restricted normals vanish.  Because the
zero-normal inequalities have nonnegative right-hand side, this does not
change the common-face coordinate polytope. -/
...
/-- Pad any presentation with at most `2*r` rows to exactly `2*r` rows by
adding tautological inequalities `0 ≤ 1`. -/
...
/-- If a common face of dimension `r` has at most `2*r` nonzero restricted
row normals, then it admits an exactly balanced `2*r`-row H-presentation
without changing its coordinate polytope. -/
...
/-- Coordinate diameter is invariant under replacing a common face by the
exactly balanced effective-row presentation above. -/
