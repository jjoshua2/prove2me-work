...
/-! Finite closed-cover connectivity for simultaneous clipping.
The closed sets are actual parent faces, not the possibly nonclosed vertex set. -/
...
/-- A finite closed cover of a preconnected trace supplies actual region
...
/-- A connected trace covered by finitely many closed parent faces routes with
one charge per face. There is no checkpoint partition assumption. -/
