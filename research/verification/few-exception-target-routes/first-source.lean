...
/-- Pad a finite set of original row labels into k optional slots. -/
noncomputable def encodeRows (k : ℕ) (T : Finset (Fin m)) : Fin k → Option (Fin m) :=
  fun i => if hi : i.val < Fintype.card T then
    some ((Fintype.equivFin T).symm ⟨i.val,hi⟩).val else none

lemma mem_encodeRows (k : ℕ) (T : Finset (Fin m)) (hT : T.card ≤ k) (a : Fin m) :
    a ∈ T ↔ ∃ i : Fin k, encodeRows k T i=some a := by
  classical
  constructor
  · intro ha
    let j := (Fintype.equivFin T) ⟨a,ha⟩
    have hjk : j.val < k := lt_of_lt_of_le j.isLt (by simpa only [Fintype.card_coe] using hT)
    refine ⟨⟨j.val,hjk⟩,?_⟩
    rw [encodeRows,dif_pos j.isLt]
    exact congrArg (fun t : T => some t.val) ((Fintype.equivFin T).symm_apply_apply ⟨a,ha⟩)
  · rintro ⟨i,hi⟩
    unfold encodeRows at hi
...
lemma encodeRows_injective (k : ℕ) (S T : Finset (Fin m))
    ...
    (he : encodeRows k S=encodeRows k T) : S=T := by
  classical
  ext i
  rw [mem_encodeRows k S hS,mem_encodeRows k T hT,he]

/-- The remaining ACTUAL vertices inject into k-slot ORIGINAL-row certificates.
This is an original-input bound, not a supplied small vertex catalogue. -/
theorem residual_card (C : Finset (Fin d → ℝ))
    (A : Fin m → (Fin d → ℝ) →ₗ[ℝ] ℝ) (b : Fin m → ℝ)
    (v : Fin d → ℝ) (hv : v ∈ (body A b).extremePoints ℝ)
    (B : Finset (Fin m)) :
    (residual C A b v B).card ≤ (m+1)^B.card := by
  classical
  let V := residual C A b v B
  let N := directions A (good A b v B)
  have hV : ∀ x : V, x.val ∈ (body A b).extremePoints ℝ ∧
      ∀ i ∈ good A b v B, A i x.val=b i := by
    intro x
    exact ⟨(Finset.mem_filter.mp (Finset.mem_filter.mp x.property).1).2,
      (Finset.mem_filter.mp x.property).2⟩
  have hchoose : ∀ x : V, ∃ T : Finset (Fin m), T.card ≤ B.card ∧
      (∀ i ∈ T, A i x.val=b i) ∧
      ∀ z : N, (∀ i ∈ T, A i z.val=0) → z.val=0 := by
    intro x
    obtain ⟨T,hT,ha,hz⟩ := small_active_rows A b N x.val (hV x).1
    exact ⟨T,hT.trans (residual_dimension A b v hv B),ha,hz⟩
  choose T hT ha hz using hchoose
  have hTin : Function.Injective T := by
    intro x y he
    apply Subtype.ext
    apply sub_eq_zero.mp
    have hdiff : x.val-y.val ∈ N := by
      intro i hi
      rw [map_sub,(hV x).2 i hi,(hV y).2 i hi,sub_self]
    apply hz x ⟨x.val-y.val,hdiff⟩
    intro i hi
    have hi' : i ∈ T y := he ▸ hi
    change A i (x.val-y.val)=0
    rw [map_sub,ha x i hi,ha y i hi',sub_self]
  let code : V → (Fin B.card → Option (Fin m)) := fun x => encodeRows B.card (T x)
  have hcode : Function.Injective code := by
    intro x y he
    apply hTin
    exact encodeRows_injective B.card (T x) (T y) (hT x) (hT y) he
  have h := Fintype.card_le_of_injective code hcode
  simpa only [Fintype.card_coe,Fintype.card_fun,Fintype.card_fin,Fintype.card_option] using h
...
/-- Finish within the remaining original face, preserving target locks.
The bound is the PROVED original-row code count, not a scalar level count. -/
theorem residual_route (C : Finset (Fin d → ℝ))
    (A : Fin m → (Fin d → ℝ) →ₗ[ℝ] ℝ) (b : Fin m → ℝ)
    (hP : convexHull ℝ (C : Set (Fin d → ℝ))=body A b)
    (v : Fin d → ℝ) (hv : v ∈ (body A b).extremePoints ℝ)
    (B : Finset (Fin m)) (u : Fin d → ℝ) (hu : u ∈ residual C A b v B) :
    ∃ p : CoordinateRoute.Route (Edge A b v) (residual C A b v B) u v,
      p.length+1 ≤ (m+1)^B.card := by
  classical
  ...
  exact ⟨p,hp.trans (residual_card C A b v hv B)⟩

/-- Several unrestricted target rows have a counted geometric finish.
For each fixed k=|B| the resulting bound is polynomial in ORIGINAL m. -/
theorem few_exception_routes (C : Finset (Fin d → ℝ))
    (A : Fin m → (Fin d → ℝ) →ₗ[ℝ] ℝ) (b : Fin m → ℝ)
    (hP : convexHull ℝ (C : Set (Fin d → ℝ))=body A b)
    (v : Fin d → ℝ) (hv : v ∈ (body A b).extremePoints ℝ)
    (B : Finset (Fin m))
    ...
    ∃ p : CoordinateRoute.Route (Edge A b v) (vertexSet C A b) u v,
      p.length ≤ m-d+((m+1)^B.card-1) := by
  classical
  obtain ⟨w,hw,p,hp⟩ := enter_residual C A b hP v hv B htwo u hu
  obtain ⟨q,hq⟩ := residual_route C A b hP v hv B w hw
  ...
  change p.length+q.length ≤ m-d+((m+1)^B.card-1)
  omega

end Hirsch.FewExceptions

/-- Original-edge routes with arbitrarily many unrestricted target rows,
controlled by their count rather than their possibly huge level inventories. -/
theorem solution (d m : ℕ) (C : Finset (Fin d → ℝ))
  ...
  classical
  obtain ⟨p,hp⟩ := Hirsch.FewExceptions.few_exception_routes C A b hP v hv B htwo u hu
...
#print axioms Hirsch.FewExceptions.residual_dimension
#print axioms Hirsch.FewExceptions.small_active_rows
#print axioms Hirsch.FewExceptions.residual_card
#print axioms Hirsch.FewExceptions.enter_residual
#print axioms Hirsch.FewExceptions.residual_route
#print axioms Hirsch.FewExceptions.few_exception_routes
