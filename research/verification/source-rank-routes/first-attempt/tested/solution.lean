...
/-- The complete actual vertex set in the CURRENT common target face. -/
noncomputable def face (C : Finset (Fin d → ℝ))
    (A : Fin m → Slope d) (b : Fin m → ℝ) (v x : Fin d → ℝ) :
    Finset (Fin d → ℝ) := by
  classical
  exact (vertexSet C A b).filter
    (fun z => ∀ i, A i v=b i → A i x=b i → A i z=b i)

def ratio (h : Slope d) (v : Fin d → ℝ) (D : Slope d) (z : Fin d → ℝ) : ℝ :=
  h (z-v)/(1+D (z-v))

def Positive (C : Finset (Fin d → ℝ)) (A : Fin m → Slope d)
    (b : Fin m → ℝ) (v x : Fin d → ℝ) (D : Slope d) : Prop :=
  ∀ z ∈ face C A b v x, 0<1+D (z-v)

/-- Only DISTINCT values strictly below the current value are charged. -/
noncomputable def lower (C : Finset (Fin d → ℝ)) (A : Fin m → Slope d)
    (b : Fin m → ℝ) (h : Slope d) (v x : Fin d → ℝ) (D : Slope d) : Finset ℝ := by
  classical
  exact ((face C A b v x).image (ratio h v D)).filter (fun a => a<ratio h v D x)

lemma source_mem (C : Finset (Fin d → ℝ)) (A : Fin m → Slope d)
    (b : Fin m → ℝ) (v x : Fin d → ℝ) (hx : x ∈ vertexSet C A b) :
    x ∈ face C A b v x := by
  classical
  exact Finset.mem_filter.mpr ⟨hx,fun _ _ hi => hi⟩

lemma edge_face (C : Finset (Fin d → ℝ)) (A : Fin m → Slope d)
    (b : Fin m → ℝ) (v x y : Fin d → ℝ) (hy : y ∈ vertexSet C A b)
    (he : Edge A b v x y) : y ∈ face C A b v x := by
  classical
  exact Finset.mem_filter.mpr ⟨hy,he.2.2⟩

lemma face_shrinks (C : Finset (Fin d → ℝ)) (A : Fin m → Slope d)
    (b : Fin m → ℝ) (v x y : Fin d → ℝ) (he : Edge A b v x y) :
    face C A b v y ⊆ face C A b v x := by
  classical
  intro z hz
  obtain ⟨hzV,hzH⟩ := Finset.mem_filter.mp hz
  exact Finset.mem_filter.mpr ⟨hzV,fun i hiv hix => hzH i hiv (he.2.2 i hiv hix)⟩

/-- The minimum is attained for every state; no proposed denominator is input.
Finite order-cell enumeration is a separate executable construction. -/
theorem minimizers (C : Finset (Fin d → ℝ)) (A : Fin m → Slope d)
    (b : Fin m → ℝ) (h : Slope d) (v : Fin d → ℝ) :
    ∃ E : (Fin d → ℝ) → Slope d,
      (∀ x, Positive C A b v x (E x)) ∧
      ∀ x, ∀ D : Slope d, Positive C A b v x D →
        (lower C A b h v x (E x)).card ≤ (lower C A b h v x D).card := by
  classical
  have hc : ∀ x : Fin d → ℝ, ∃ D : Slope d, Positive C A b v x D ∧
      ∀ F : Slope d, Positive C A b v x F →
        (lower C A b h v x D).card ≤ (lower C A b h v x F).card := by
    intro x
    have hz : Positive C A b v x (0 : Slope d) := by
      intro z hz
      simp
    have hex : ∃ n : ℕ, ∃ D : Slope d, Positive C A b v x D ∧
        (lower C A b h v x D).card=n := ⟨_,0,hz,rfl⟩
    obtain ⟨D,hD,hd⟩ := Nat.find_spec hex
    refine ⟨D,hD,?_⟩
    intro F hF
    rw [hd]
    exact Nat.find_min' hex ⟨F,hF,rfl⟩
  choose E hE hmin using hc
  exact ⟨E,hE,hmin⟩

/-- Old-ratio descent and actual face shrinkage strictly lower the old rank. -/
lemma lower_decreases (C : Finset (Fin d → ℝ)) (A : Fin m → Slope d)
    (b : Fin m → ℝ) (h : Slope d) (v x y : Fin d → ℝ) (D : Slope d)
    (hy : y ∈ vertexSet C A b) (he : Edge A b v x y)
    (hdec : ratio h v D y < ratio h v D x) :
    (lower C A b h v y D).card < (lower C A b h v x D).card := by
  classical
  have hfaces := face_shrinks C A b v x y he
  ...
  · intro a ha
    ...
    exact Finset.mem_filter.mpr
      ⟨Finset.mem_image.mpr ⟨z,hfaces hz,rfl⟩,haL.trans hdec⟩
  · intro heq
    have hym : ratio h v D y ∈ lower C A b h v x D :=
      Finset.mem_filter.mpr
        ⟨Finset.mem_image.mpr ⟨y,edge_face C A b v x y hy he,rfl⟩,hdec⟩
    rw [← heq] at hym
    exact (lt_irrefl (ratio h v D y)) (Finset.mem_filter.mp hym).2

/-- Derive a genuine original edge that lowers the REOPTIMIZED rank.
No target row has to be acquired in this particular step. -/
theorem decreasing_edge (C : Finset (Fin d → ℝ)) (A : Fin m → Slope d)
    (b : Fin m → ℝ) (hP : convexHull ℝ (C : Set (Fin d → ℝ))=body A b)
    (h : Slope d) (v : Fin d → ℝ) (hv : v ∈ (body A b).extremePoints ℝ)
    (hh : ∀ z ∈ vertexSet C A b, z ≠ v → 0<h (z-v))
    (E : (Fin d → ℝ) → Slope d)
    (hE : ∀ x, Positive C A b v x (E x))
    (hmin : ∀ x, ∀ D : Slope d, Positive C A b v x D →
      (lower C A b h v x (E x)).card ≤ (lower C A b h v x D).card)
    (x : Fin d → ℝ) (hx : x ∈ (body A b).extremePoints ℝ) (hxv : x ≠ v) :
    ∃ y ∈ (body A b).extremePoints ℝ, Edge A b v x y ∧
      (lower C A b h v y (E y)).card < (lower C A b h v x (E x)).card := by
  classical
  have hmem : ∀ z ∈ (body A b).extremePoints ℝ, z ∈ vertexSet C A b := by
    intro z hz
    exact Finset.mem_filter.mpr ⟨extremePoints_convexHull_subset (hP.symm ▸ hz),hz⟩
  have hxF := source_mem C A b v x (hmem x hx)
  let D := E x
  let r := ratio h v D x
  have hr : 0<r := div_pos (hh x (hmem x hx) hxv) (hE x x hxF)
  have heq : r*(1+D (x-v))=h (x-v) := div_mul_cancel₀ _ (ne_of_gt (hE x x hxF))
  rw [map_sub,map_sub] at heq
  let f : Slope d := -h+r • D
  have hfxv : f x < f v := by
    change -h x+r*D x < -h v+r*D v
    nlinarith
  obtain ⟨y,hy,hxy,hinc⟩ := improve_locked C A b hP v hv x hx f hfxv
  have hyF := edge_face C A b v x y (hmem y hy) hxy
  have hratio : ratio h v D y < ratio h v D x := by
    change h (y-v)/(1+D (y-v)) < r
    apply (div_lt_iff₀ (hE x y hyF)).mpr
    rw [map_sub,map_sub]
    change -h x+r*D x < -h y+r*D y at hinc
    nlinarith
  have hD : Positive C A b v y D := by
    intro z hz
    exact hE x z (face_shrinks C A b v x y hxy hz)
  exact ⟨y,hy,hxy,(hmin y D hD).trans_lt
    (lower_decreases C A b h v x y D (hmem y hy) hxy hratio)⟩

lemma target_rank_zero (C : Finset (Fin d → ℝ)) (A : Fin m → Slope d)
    (b : Fin m → ℝ) (h : Slope d) (v : Fin d → ℝ)
    (hh : ∀ z ∈ vertexSet C A b, z ≠ v → 0<h (z-v))
    (D : Slope d) (hD : Positive C A b v v D) :
    (lower C A b h v v D).card=0 := by
  classical
  have he : lower C A b h v v D=∅ := by
    ext a
    constructor
    · intro ha
      obtain ⟨haI,haL⟩ := Finset.mem_filter.mp ha
      obtain ⟨z,hz,rfl⟩ := Finset.mem_image.mp haI
      have hz0 : 0 ≤ ratio h v D z := by
        by_cases hzv : z=v
        · simp [ratio,hzv]
        · exact (div_pos (hh z (Finset.mem_filter.mp hz).1 hzv) (hD z hz)).le
      have hv0 : ratio h v D v=0 := by simp [ratio]
      rw [hv0] at haL
      exact False.elim ((not_lt_of_ge hz0) haL)
    · simp
  rw [he,Finset.card_empty]

/-- Assemble the decreasing edges; the potential never resets between phases. -/
theorem ranked_route (C : Finset (Fin d → ℝ)) (A : Fin m → Slope d)
    (b : Fin m → ℝ) (hP : convexHull ℝ (C : Set (Fin d → ℝ))=body A b)
    (h : Slope d) (v : Fin d → ℝ) (hv : v ∈ (body A b).extremePoints ℝ)
    (hh : ∀ z ∈ vertexSet C A b, z ≠ v → 0<h (z-v))
    (E : (Fin d → ℝ) → Slope d)
    ...
    ∃ p : CoordinateRoute.Route
        (fun x y => Edge A b v x y ∧
          (lower C A b h v y (E y)).card < (lower C A b h v x (E x)).card)
        (vertexSet C A b) u v,
      p.length ≤ (lower C A b h v u (E u)).card := by
  classical
  have hmem : ∀ z ∈ (body A b).extremePoints ℝ, z ∈ vertexSet C A b := by
    intro z hz
    exact Finset.mem_filter.mpr ⟨extremePoints_convexHull_subset (hP.symm ▸ hz),hz⟩
  let rho := fun x => (lower C A b h v x (E x)).card
  have aux : ∀ n : ℕ, ∀ x : Fin d → ℝ, x ∈ (body A b).extremePoints ℝ → rho x=n →
      ∃ p : CoordinateRoute.Route (fun x y => Edge A b v x y ∧ rho y<rho x)
        (vertexSet C A b) x v, p.length ≤ rho x := by
    intro n
    ...
    | h n ih =>
      ...
      obtain ⟨y,hy,hxy,hdec⟩ := decreasing_edge C A b hP h v hv hh E hE hmin x hx hxv
      obtain ⟨p,hp⟩ := ih (rho y) (by change rho y<rho x at hdec; omega) y hy rfl
      refine ⟨CoordinateRoute.Route.prepend (hmem x hx) ⟨hxy,hdec⟩ p,?_⟩
      change p.length+1 ≤ rho x
      change rho y<rho x at hdec
      omega
  exact aux (rho u) u hu rfl

end Hirsch.SourceRank

/-- A source-sensitive optimized rank decreases at EVERY original edge.
The exposure, local normalizers, minima and complete route are derived. -/
theorem solution (d m : ℕ) (C : Finset (Fin d → ℝ))
          ...
          ∃ p : Fin (L+1) → (Fin d → ℝ), p 0=u ∧ p (Fin.last L)=v ∧
            ...
            ∀ t : Fin L,
              ...
              (R (p t.succ) (E (p t.succ))).card <
                (R (p t.castSucc) (E (p t.castSucc))).card := by
  classical
  let V := @Finset.filter (Fin d → ℝ)
    (fun x => x ∈ ({x | ∀ i, A i x ≤ b i} : Set (Fin d → ℝ)).extremePoints ℝ)
    (fun _ => Classical.propDecidable _) C
  ...
  have hF : ∀ x, Hirsch.SourceRank.face C A b v x=F x := by
    intro x
    ext z
    simp only [Hirsch.SourceRank.face,F,Finset.mem_filter,hV]
  obtain ⟨h,hh⟩ := Hirsch.HullCoordinate.strict_vertex_functional C v (hP.symm ▸ hv)
  have hhV : ∀ z ∈ Hirsch.TargetRows.vertexSet C A b, z ≠ v → 0<h (z-v) := by
    intro z hz hne
    exact hh z (Finset.mem_filter.mp hz).1 hne
  obtain ⟨E,hE,hmin⟩ := Hirsch.SourceRank.minimizers C A b h v
  let R := fun (x : Fin d → ℝ) (D : (Fin d → ℝ) →ₗ[ℝ] ℝ) =>
    @Finset.filter ℝ (fun a => a<h (x-v)/(1+D (x-v)))
      (fun _ => Classical.propDecidable _)
      ((F x).image (fun z => h (z-v)/(1+D (z-v))))
  have hR : ∀ x D, Hirsch.SourceRank.lower C A b h v x D=R x D := by
    intro x D
    ext a
    simp only [Hirsch.SourceRank.lower,Hirsch.SourceRank.ratio,R,
      Finset.mem_filter,Finset.mem_image,hF]
  obtain ⟨p,hp⟩ := Hirsch.SourceRank.ranked_route C A b hP h v hv hhV E hE hmin u hu
  refine ⟨h,hh,E,?_,?_,?_,p.length,?_,?_,fun t => p.point t.val,p.first,p.last,?_,?_⟩
  · intro x hx z hz
    exact hE x z (by simpa only [hF] using hz)
  · intro x hx D hD
    have hc := hmin x D (by simpa only [Hirsch.SourceRank.Positive,hF] using hD)
    simpa only [hR] using hc
  · simpa only [hR] using Hirsch.SourceRank.target_rank_zero C A b h v hhV (E v) (hE v)
  · simpa only [hR] using hp
  · intro D hD
    have hc := hmin u D (by simpa only [Hirsch.SourceRank.Positive,hF] using hD)
    simpa only [hR] using hp.trans hc
  · intro t
    exact (Finset.mem_filter.mp (p.mem t.val (by have ht := t.isLt; omega))).2
  · intro t
    have ht := p.step t.val t.isLt
    exact ⟨ht.1,by simpa only [hR] using ht.2⟩

#print axioms Hirsch.HullCoordinate.strict_vertex_functional
#print axioms Hirsch.SourceRank.minimizers
#print axioms Hirsch.SourceRank.lower_decreases
#print axioms Hirsch.SourceRank.decreasing_edge
#print axioms Hirsch.SourceRank.target_rank_zero
#print axioms Hirsch.SourceRank.ranked_route
