...
/-- Every original tight row at any actual moment vertex has a constructed
first-blocking pivot and a genuine exposed original-edge neighbor. -/
theorem solution (d m : ℕ) (hm : d < m) (a : Fin m → ℝ)
    ...
    u ∈ P.extremePoints ℝ → ∀ p ∈ I,
      ∃ (w : Fin d → ℝ) (t : ℝ) (q : Fin m),
        ...
        (∀ s : ℝ, 0 ≤ s → ((u+s • w ∈ P) ↔ s ≤ t)) := by
  classical
  dsimp only
  intro hu p hp
  exact Hirsch.MomentRelease.release_edge a ha hm u hu p hp

#print axioms Hirsch.MomentVertices.activeEval_surjective
#print axioms Hirsch.MomentRelease.first_blocker
#print axioms Hirsch.MomentRelease.release_direction
