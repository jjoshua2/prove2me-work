...
lemma squareRoots_degree {ι : Type*} (a : ι → ℝ) (S : Finset ι) :
  ...
  classical
  calc
    (squareRoots a S).natDegree =
        ∑ s ∈ S, ((Polynomial.X - Polynomial.C (a s)) ^ 2).natDegree :=
      Polynomial.natDegree_prod_of_monic S _
        (fun s _ => (Polynomial.monic_X_sub_C (a s)).pow 2)
    _ = 2 * S.card := by simp [Polynomial.natDegree_pow, Nat.mul_comm]
