...
/-- Optimize over a nonempty DERIVED family of small determining completions.
This is finite mathematical minimization, not a polynomial-time algorithm claim. -/
theorem minimum_completion (C : Finset (Fin d → ℝ))
    (A : Fin m → (Fin d → ℝ) →ₗ[ℝ] ℝ) (b : Fin m → ℝ)
    (u v : Fin d → ℝ) (hv : v ∈ (body A b).extremePoints ℝ) :
    ∃ S : Finset (Fin m), S ⊆ missing A b v u ∧ S.card ≤ d ∧
      Determines A (shared A b u v) S ∧
      ∀ R : Finset (Fin m), R ⊆ missing A b v u → R.card ≤ d →
        Determines A (shared A b u v) R → weight C A b S ≤ weight C A b R := by
  classical
  ...
  obtain ⟨S,hSF,hmin⟩ := Finset.exists_min_image F (weight C A b) ⟨R,hRF⟩
...
noncomputable def due (A : Fin m → (Fin d → ℝ) →ₗ[ℝ] ℝ)
    (b : Fin m → ℝ) (S : Finset (Fin m)) (x : Fin d → ℝ) : Finset (Fin m) := by
  classical
  exact S.filter (fun i => A i x ≠ b i)

noncomputable def cost (C : Finset (Fin d → ℝ))
    (A : Fin m → (Fin d → ℝ) →ₗ[ℝ] ℝ) (b : Fin m → ℝ)
    (S : Finset (Fin m)) (x : Fin d → ℝ) : ℕ :=
  ∑ i ∈ due A b S x, ((levels C A b i).card-1)

lemma due_after_phase (C : Finset (Fin d → ℝ))
    (A : Fin m → (Fin d → ℝ) →ₗ[ℝ] ℝ) (b : Fin m → ℝ)
    (v x y : Fin d → ℝ) (S : Finset (Fin m))
    (hS : ∀ i ∈ S, A i v=b i)
    (p : CoordinateRoute.Route (Edge A b v) (vertexSet C A b) x y)
    (j : Fin m) (hjy : A j y=b j) :
    due A b S y ⊆ (due A b S x).erase j := by
  classical
  intro i hi
...
lemma cost_after_phase (C : Finset (Fin d → ℝ))
    (A : Fin m → (Fin d → ℝ) →ₗ[ℝ] ℝ) (b : Fin m → ℝ)
    (v x y : Fin d → ℝ) (S : Finset (Fin m))
    (hS : ∀ i ∈ S, A i v=b i)
    (p : CoordinateRoute.Route (Edge A b v) (vertexSet C A b) x y)
    (j : Fin m) (hj : j ∈ due A b S x) (hjy : A j y=b j) :
    cost C A b S y+((levels C A b j).card-1) ≤ cost C A b S x := by
  classical
  let w : Fin m → ℕ := fun i => (levels C A b i).card-1
  have hsub := due_after_phase C A b v x y S hS p j hjy
  have hsum : (∑ i ∈ due A b S y, w i) ≤ ∑ i ∈ (due A b S x).erase j, w i :=
    Finset.sum_le_sum_of_subset_of_nonneg hsub (by intro i _ _; exact Nat.zero_le _)
  have heq : (∑ i ∈ (due A b S x).erase j, w i)+w j =
      ∑ i ∈ due A b S x, w i := Finset.sum_erase_add _ _ hj
  change (∑ i ∈ due A b S y, w i)+w j ≤ ∑ i ∈ due A b S x, w i
  omega

/-- Construct phases ONLY for the derived determining rows. Common equations
stay locked, so no unselected target row has to be purchased as a separate phase. -/
theorem selected_routes (C : Finset (Fin d → ℝ))
  ...
  have hmem : ∀ x ∈ (body A b).extremePoints ℝ, x ∈ vertexSet C A b := by
    ...
    exact Finset.mem_filter.mpr ⟨extremePoints_convexHull_subset (hP.symm ▸ hx),hx⟩
  have aux : ∀ k : ℕ, ∀ x : Fin d → ℝ, x ∈ (body A b).extremePoints ℝ →
      (∀ i ∈ G, A i x=b i) → (due A b S x).card=k →
      ∃ p : CoordinateRoute.Route (Edge A b v) (vertexSet C A b) x v,
        p.length ≤ cost C A b S x := by
    intro k
    induction k using Nat.strong_induction_on with
    | h k ih =>
      intro x hx hxG hk
      ...
      have hj : j ∈ due A b S x := Finset.mem_filter.mpr ⟨hjS,hjx⟩
      obtain ⟨y,hy,hjy,p,hp⟩ := acquire_row C A b hP v hv j (hS j hjS) x hx
      have hyG : ∀ i ∈ G, A i y=b i := by
        intro i hi
        exact route_preserves C A b v x y p i (hGv i hi) (hxG i hi)
      have hsub := due_after_phase C A b v x y S hS p j hjy
      have hdec : (due A b S y).card < (due A b S x).card :=
        lt_of_le_of_lt (Finset.card_le_card hsub) (Finset.card_erase_lt_of_mem hj)
      obtain ⟨q,hq⟩ := ih (due A b S y).card (by omega) y hy hyG rfl
      have hb := cost_after_phase C A b v x y S hS p j hj hjy
      refine ⟨p.append q,?_⟩
      change p.length+q.length ≤ cost C A b S x
      omega
  obtain ⟨p,hp⟩ := aux (due A b S u).card u hu hGu rfl
  have hDue : due A b S u=S := by
    apply Finset.filter_eq_self.mpr
    intro i hi
    exact ((mem_missing A b v u i).mp (hSsub hi)).2
  exact ⟨p,by simpa only [cost,hDue,weight] using hp⟩

lemma weight_uniform (C : Finset (Fin d → ℝ))
    (A : Fin m → (Fin d → ℝ) →ₗ[ℝ] ℝ) (b : Fin m → ℝ)
    (u v : Fin d → ℝ) (hu : u ∈ (body A b).extremePoints ℝ)
    (S : Finset (Fin m)) (hSsub : S ⊆ missing A b v u) (hScard : S.card ≤ d)
    (K : ℕ) (hK : ∀ i ∈ S, (levels C A b i).card ≤ K+1) :
    weight C A b S ≤ K*min d (m-d) := by
  classical
  have hcard : S.card ≤ min d (m-d) := le_min hScard
    ((Finset.card_le_card hSsub).trans (missing_bound A b u v hu))
  have hsum : weight C A b S ≤ K*S.card := by
    calc
      weight C A b S = ∑ i ∈ S, ((levels C A b i).card-1) := rfl
      _ ≤ ∑ _i ∈ S, K := by
        apply Finset.sum_le_sum
        intro i hi
        have hh := hK i hi
...
/-- Existence, optimal weighted selection, and the SAME original walk.
Only the finite family of determining row sets is minimized, not path length. -/
theorem optimal_routes (C : Finset (Fin d → ℝ))
    ...
    ∃ S : Finset (Fin m), S ⊆ missing A b v u ∧ S.card ≤ d ∧
      Determines A (shared A b u v) S ∧
      ∃ p : CoordinateRoute.Route (Edge A b v) (vertexSet C A b) u v,
        p.length ≤ weight C A b S ∧
        (∀ R : Finset (Fin m), R ⊆ missing A b v u → R.card ≤ d →
          Determines A (shared A b u v) R → weight C A b S ≤ weight C A b R) ∧
        (∀ K : ℕ, (∀ i ∈ S, (levels C A b i).card ≤ K+1) →
          p.length ≤ K*min d (m-d)) := by
  classical
  obtain ⟨S,hSsub,hScard,hSdet,hmin⟩ := minimum_completion C A b u v hv
  obtain ⟨p,hp⟩ := selected_routes C A b hP u v hu hv S hSsub hSdet
  refine ⟨S,hSsub,hScard,hSdet,p,hp,?_,?_⟩
  · intro R hRsub hRcard hRdet
    exact hmin R hRsub hRcard hRdet
  · intro K hK
    exact hp.trans (weight_uniform C A b u v hu S hSsub hScard K hK)

end Hirsch.TargetDetermining

/-- Derive and optimize a small ORIGINAL target-determining row set, then
construct a locked original-edge route paying only for its selected rows. -/
theorem solution (d m : ℕ) (C : Finset (Fin d → ℝ))
  ...
  obtain ⟨S,hSsub,hScard,hSdet,p,hp,hopt,huniform⟩ :=
    Hirsch.TargetDetermining.optimal_routes C A b hP u v hu hv
  refine ⟨S,?_,hScard,?_,p.length,?_,?_,?_,fun t => p.point t.val,p.first,p.last,?_,?_⟩
  · simpa only [hT] using hSsub
  · simpa only [Hirsch.TargetDetermining.Determines,hG] using hSdet
  · simpa only [Hirsch.TargetDetermining.weight,Hirsch.TargetLevels.levels,hV] using hp
  · intro R hRsub hRcard hRdet
    ...
    simpa only [Hirsch.TargetDetermining.weight,Hirsch.TargetLevels.levels,hV] using
      hopt R hsub hRcard hdet
  · intro K hK
    apply huniform K
    intro i hi
...
#print axioms Hirsch.FewExceptions.small_active_rows
#print axioms Hirsch.TargetDetermining.small_completion
#print axioms Hirsch.TargetDetermining.minimum_completion
#print axioms Hirsch.TargetDetermining.cost_after_phase
#print axioms Hirsch.TargetDetermining.selected_routes
#print axioms Hirsch.TargetDetermining.weight_uniform
#print axioms Hirsch.TargetDetermining.optimal_routes
