...
## Scope and parallel-agent handoff

The user has assigned #201 verification and Prove2Me submission to another
agent. This continuation takes that mathematics as available and does not
change its branch, trigger its workflow, or redo its publication.

Base main: `65dc76dd2a2e9cc01a3e0ae3f64464d4eebfbdd3`.
The two new Lean modules are independent of #201's imports: the coarse
support-deficit corollary already follows from merged #193. They retain the
same certificate and actual selected parent-vertex portal pairs from #200.
The exact-degree refinements below use #201. No new open child is registered.

New source is a proof candidate, not a compiler or platform receipt. The
mathematical arguments are given below. New local verification commands are:

```
lake build Solutions.PolynomialVertexCarrierIntrinsicBudget Solutions.PolynomialSupportDeficitRouting
```

Keep Lean 4.30.0 / Mathlib c5ea003 and the explicit already-public Larman and
small-excess inputs. Inspect all new transitive axiom printouts before merge.
No Actions workflow, pin, credential or Prove2Me record was changed here.
