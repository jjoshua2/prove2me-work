...
private lemma prod_erase_pair {I : Type*} [Fintype I] [DecidableEq I]
  ...
  rw [hs, Finset.prod_union hd, Finset.prod_singleton, Finset.prod_product]
...
theorem solution (k m : ℕ) (a : Fin m → ℝ) (ha : Function.Injective a)
  ...
  · intro S hs
    have hSk : S.card ≤ k := by
      ...
      · have h := Finset.card_lt_card hs; omega
      · have h := Finset.card_lt_card hs; omega
    have hkm : k+1 ≤ m := by
    ...
    refine ⟨x,?_⟩
    intro i
    simpa only [Fintype.card_fin] using ⟨(hx i).2.1,(hx i).2.2⟩
