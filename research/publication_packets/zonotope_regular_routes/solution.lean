...
theorem event_mesh (T : Finset ℝ) (hT : ∀ x ∈ T, 0 < x ∧ x < 1) :
  ...
  have hcard : S.card = T.card+2 := by
    rw [S, Finset.card_insert_of_notMem h0', Finset.card_insert_of_notMem h1]
    omega
...
lemma segment_eq_of_extremes {d : ℕ} (P : Set (Fin d → ℝ))
  ...
  rcases extreme_on_segment P u v x hu hv hx hxs with rfl | rfl <;>
    rcases extreme_on_segment P u v y hu hv hy hys with rfl | rfl
