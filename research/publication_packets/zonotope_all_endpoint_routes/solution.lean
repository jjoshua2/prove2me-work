...
namespace Hirsch.ZonotopeAllPairs

open Set ZonotopeWall ZonotopeWalk
open scoped Pointwise

/-- All Boolean corner images, without an injectivity or vertex assumption. -/
def corner {d m : ℕ} (w : Fin m → (Fin d → ℝ)) (s : Fin m → Bool) : Fin d → ℝ :=
  ∑ i : Fin m, if s i then w i else 0

def corners {d m : ℕ} (w : Fin m → (Fin d → ℝ)) : Set (Fin d → ℝ) :=
  Set.range (corner w)

lemma corners_finite {d m : ℕ} (w : Fin m → (Fin d → ℝ)) : (corners w).Finite :=
  Set.finite_range _

lemma corners_eq_sum {d m : ℕ} (w : Fin m → (Fin d → ℝ)) :
    corners w = ∑ i : Fin m, ({0, w i} : Set (Fin d → ℝ)) := by
  classical
  ext x
  simp only [Set.mem_finsetSum, Finset.mem_univ, forall_true_left]
  constructor
  · rintro ⟨s, rfl⟩
    refine ⟨fun i => if s i then w i else 0, ?_, rfl⟩
    intro i
    cases hi : s i <;> simp [hi]
  · rintro ⟨a, ha, rfl⟩
    refine ⟨fun i => decide (a i = w i), ?_⟩
    apply Finset.sum_congr rfl
    intro i _
    have hai : a i = 0 ∨ a i = w i := by simpa using @ha i
    rcases hai with hi | hi
    · by_cases hw : w i = 0
      · simp [hi, hw]
      · have hw' : (0 : Fin d → ℝ) ≠ w i := Ne.symm hw
        simp [hi, hw']
    · simp [hi]

lemma body_eq_parallelepiped {d m : ℕ} (w : Fin m → (Fin d → ℝ)) :
    body w = parallelepiped w := by
  ext x
  constructor
  · rintro ⟨s, hs, rfl⟩
    exact ⟨s, ⟨fun i => (hs i).1, fun i => (hs i).2⟩, rfl⟩
  · rintro ⟨s, hs, he⟩
    exact ⟨s, fun i => ⟨hs.1 i, hs.2 i⟩, he⟩

/-- The original segment sum is the hull of a derived finite corner set. -/
theorem body_eq_corner_hull {d m : ℕ} (w : Fin m → (Fin d → ℝ)) :
    body w = convexHull ℝ (corners w) := by
  rw [body_eq_parallelepiped, parallelepiped_eq_convexHull, corners_eq_sum]

/-- Separate a requested actual vertex from the hull of every other corner
image. No exposing objective or vertex catalogue is supplied. -/
theorem corner_unique_objective {d m : ℕ} (w : Fin m → (Fin d → ℝ))
    (u : Fin d → ℝ) (hu : u ∈ (body w).extremePoints ℝ) :
    ∃ f : (Fin d → ℝ) →ₗ[ℝ] ℝ,
      ∀ x ∈ corners w, f x ≤ f u ∧ (f x = f u → x = u) := by
  classical
  have hC : corners w ⊆ body w := by
    rw [body_eq_corner_hull]
    exact subset_convexHull ℝ _
  have hcv : Convex ℝ (body w) := by
    rw [body_eq_corner_hull]
    exact convex_convexHull ℝ _
  have hremove : Convex ℝ (body w \ {u}) :=
    (hcv.mem_extremePoints_iff_convex_diff.mp hu).2
  have hsub : convexHull ℝ (corners w \ {u}) ⊆ body w \ {u} :=
    convexHull_min (fun x hx => ⟨hC hx.1, hx.2⟩) hremove
  have hnot : u ∉ convexHull ℝ (corners w \ {u}) := by
    intro h
    exact (hsub h).2 (Set.mem_singleton u)
  have hfinite : (corners w \ {u}).Finite :=
    (corners_finite w).subset Set.diff_subset
  ...
  have hx' : x ∈ corners w \ {u} := ⟨hx, by simpa only [Set.mem_singleton_iff] using hxu⟩
  have hlt : f x < f u := (hfc x (subset_convexHull ℝ _ hx')).trans hcu
  exact ⟨hlt.le, fun he => False.elim (hlt.ne he)⟩

lemma binary_point_mem_corners {d m : ℕ} (w : Fin m → (Fin d → ℝ))
    (s : Fin m → ℝ) (hs : ∀ i, s i = 0 ∨ s i = 1) : point w s ∈ corners w := by
  classical
  refine ⟨fun i => decide (s i = 1), ?_⟩
  apply Finset.sum_congr rfl
  intro i _
  rcases hs i with hi | hi <;> simp [hi]

lemma vertex_mem_corners {d m : ℕ} (w : Fin m → (Fin d → ℝ))
    (f : (Fin d → ℝ) →ₗ[ℝ] ℝ) : vertex w f ∈ corners w := by
  apply binary_point_mem_corners
  intro i
  unfold pick
  split_ifs <;> simp

lemma toggle_mem_corners {d m : ℕ} (w : Fin m → (Fin d → ℝ))
    (f : (Fin d → ℝ) →ₗ[ℝ] ℝ) (i : Fin m) (hi : f (w i) = 0) :
    vertex w f + w i ∈ corners w := by
  classical
  let t : Fin m → ℝ := fun j => pick (f (w j))
  have hti : t i = 0 := by simp [t, pick, hi]
  have hbinary : ∀ j, Function.update t i 1 j = 0 ∨ Function.update t i 1 j = 1 := by
    intro j
    by_cases hji : j = i
    · subst j
      simp
  ...
  have hd : point w (Function.update t i 1) - point w t = w i := by
    ...
    · intro hnot
      exact False.elim (hnot (Finset.mem_univ i))
  have he : point w (Function.update t i 1) = vertex w f + w i := by
    change point w (Function.update t i 1) = point w t + w i
    rw [← hd]
    abel
  exact he ▸ binary_point_mem_corners w _ hbinary

/-- Every actual zonotope vertex has a regular objective exposing exactly it.
A nonzero tied generator would produce a distinct maximizing corner image. -/
theorem extreme_regular_objective {d m : ℕ} (w : Fin m → (Fin d → ℝ))
    (u : Fin d → ℝ) (hu : u ∈ (body w).extremePoints ℝ) :
    ∃ f : (Fin d → ℝ) →ₗ[ℝ] ℝ,
      (∀ i, w i ≠ 0 → f (w i) ≠ 0) ∧ face w f = {u} := by
  obtain ⟨f, hmax⟩ := corner_unique_objective w u hu
  have hvc := vertex_mem_corners w f
  have hvu := hmax (vertex w f) hvc
  have huv : f u ≤ f (vertex w f) := by
    obtain ⟨s, hs, he⟩ := hu.1
    have hb := point_bound w f s hs
    rw [he] at hb
    exact hb.trans_eq (vertex_mem w f).2.symm
  have hvertex : vertex w f = u := hvu.2 (le_antisymm hvu.1 huv)
  have hreg : ∀ i, w i ≠ 0 → f (w i) ≠ 0 := by
    intro i hwi hfi
    have hc := toggle_mem_corners w f i hfi
    have he : f (vertex w f + w i) = f u := by
      rw [map_add, hfi, add_zero, hvertex]
    have hz := (hmax _ hc).2 he
    have hz' : u + w i = u := by simpa only [hvertex] using hz
    have hz'' : u + w i = u + 0 := hz'.trans (add_zero u).symm
    exact hwi (add_left_cancel hz'')
  refine ⟨f, hreg, ?_⟩
  simpa only [hvertex] using (vertex_regular w f hreg).2

/-- The endpoint-objective hypotheses of the accepted route are discharged
for any independently specified pair of actual vertices. -/
theorem all_endpoint_route {d m : ℕ} (w : Fin m → (Fin d → ℝ))
    (u v : Fin d → ℝ) (hu : u ∈ (body w).extremePoints ℝ)
    (hv : v ∈ (body w).extremePoints ℝ) :
    ∃ L : ℕ, L ≤ m ∧ ∃ p : Fin (L+1) → (Fin d → ℝ),
      ...
      ∀ i : Fin L, p i.castSucc ≠ p i.succ ∧
        IsExposed ℝ (body w) (segment ℝ (p i.castSucc) (p i.succ)) ∧
        IsExtreme ℝ (body w) (segment ℝ (p i.castSucc) (p i.succ)) := by
  obtain ⟨f, hf, hfu⟩ := extreme_regular_objective w u hu
  obtain ⟨g, hg, hgv⟩ := extreme_regular_objective w v hv
  obtain ⟨L, hL, p, hp0, hpL, hpv, hpe⟩ := regular_objective_route w f g hf hg
  have he0 : p 0 = u := by
    have h : p 0 ∈ face w f := by rw [hp0]; exact Set.mem_singleton _
    simpa only [hfu, Set.mem_singleton_iff] using h
  have heL : p (Fin.last L) = v := by
    have h : p (Fin.last L) ∈ face w g := by rw [hpL]; exact Set.mem_singleton _
    simpa only [hgv, Set.mem_singleton_iff] using h
  exact ⟨L, hL, p, he0, heL, hpv, hpe⟩

end Hirsch.ZonotopeAllPairs

/-- An all-endpoint bound in original edges, without supplied objectives. -/
...
#print axioms Hirsch.ZonotopeAllPairs.body_eq_corner_hull
#print axioms Hirsch.ZonotopeAllPairs.corner_unique_objective
#print axioms Hirsch.ZonotopeAllPairs.extreme_regular_objective
#print axioms Hirsch.ZonotopeAllPairs.all_endpoint_route
