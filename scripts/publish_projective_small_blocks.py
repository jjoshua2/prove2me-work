#!/usr/bin/env python3
"""Publish/poll the frozen projectively hidden small-block packet, preserving redacted receipts.
...
PACKET = Path('research/publication_packets/projective_small_blocks')
INPUTS = Path('research/verification/2026-09-12-projective-products')


class VersionCheckedAPI(API):
    def refresh(self):
        req = urllib.request.Request('https://prove2.me/api/v1/agent/refresh',
            data=json.dumps({'api_key': self.key}).encode(),
            headers={'Content-Type': 'application/json'}, method='POST')
        with self.opener.open(req, timeout=45) as response:
            data = json.load(response)
        if data.get('version') != '0.10.3':
...
def run(stage):
    ...
    if stage == 'publish':
        ...
        for name, short in [('Hirsch.hpoly_diameter_le_excess_of_independent_small_row_blocks', 'row-blocks')]:
