#!/usr/bin/env python3
"""Offline regression of the exact publisher refresh method; no credentials/network.

Extract only the real VersionCheckedAPI class and execute it against a stub API
and JSON opener. This tests the production method rather than a copied model.
The immutable source hashes make any unrelated publisher edit fail this review.
"""
...
OLD_BLOB = '099844737f4f8d61a4ea26e28967bf464959673c'
NEW_BLOB = '80a674c8a31a37e0122d0bcdf43c17a389e418de'
...
class ProtocolTests(unittest.TestCase):
    def test_exact_single_literal_change(self):
        ...
        self.assertEqual(data.count(b"'0.10.8'"), 1)
        self.assertEqual(git_blob(data.replace(b"'0.10.8'", b"'0.10.7'")), OLD_BLOB)

    def test_accepts_reviewed_version(self):
        instance = client({'version': '0.10.8', 'access_token': 'dummy-offline-token', 'expires_at': 1234})
        instance.refresh()
    ...
    def test_rejects_unreviewed_versions_before_token_install(self):
        for version in ['0.10.7', '0.10.6', '0.10.9', '1.0.0', '', None, 108, ['0.10.8'], '0.10.8 ']:
    ...
    def test_missing_token_does_not_succeed(self):
        instance = client({'version': '0.10.8'})
        with self.assertRaises(KeyError):
            instance.refresh()
        self.assertEqual(instance.token, '')

    def test_existing_default_expiry_behavior(self):
        instance = client({'version': '0.10.8', 'access_token': 'dummy-offline-token'})
