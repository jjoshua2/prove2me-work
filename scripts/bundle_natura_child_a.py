#!/usr/bin/env python3
"""Flatten the local Child-A proof dependency graph into one Prove2Me-ready Lean file.

Local `Solutions.*` imports are recursively inlined in dependency order. External
imports (Definitions/Mathlib) are collected at the top. Axiom-audit commands in
intermediate modules are removed; the final file prints the axioms of `solution`.
"""
...
def build_flat(entry: str) -> tuple[str, list[str]]:
    order = topo(entry)
    external: list[str] = []
    bodies: list[str] = []
    for name in order:
        ...
        for line in module_path(name).read_text().splitlines():
            ...
            if AUDIT.match(line):
                continue
            lines.append(line)
        bodies.append(f"/- BEGIN inlined Solutions.{name} -/\n" + "\n".join(lines) +
                      f"\n/- END inlined Solutions.{name} -/")
    text = "\n".join(external) + "\n\n" + "\n\n".join(bodies) + "\n\n#print axioms solution\n"
    # Comments may mention `sorry`; only inspect executable lines after stripping comments roughly.
...
def main() -> int:
    ...
    ap.add_argument("--output", type=Path, default=SOL / "NaturaChildAFlat.lean")
    ap.add_argument("--check", action="store_true")
    args = ap.parse_args()
    text, order = build_flat(args.entry)
    ...
    if args.check:
        ...
        run = subprocess.run([str(lake), "env", "lean", "-DautoImplicit=false", str(output)],
