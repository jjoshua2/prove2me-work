...
PROVED_NAMES = [
    # Completed Santos / reduction frontier.
    ...
    # Circuit Child A and reusable helpers.
...
# Definition and theorem publications whose public name is awkward or was
# historically recorded primarily by stable platform ID.
EXPECTED_IDS = {
...
def main():
    ...
    for name in OPEN_NAMES:
        ...
        if len(items) != 1:
            failures.append(f"{name}: expected exactly one live theorem, found {len(items)}")
        elif items[0].get("status") != "Open":
            failures.append(f"{name}: expected Open frontier, got {items[0].get('status')}")
    ...
    for label,(tid,expected) in EXPECTED_IDS.items():
        ...
        if item.get("status") != expected:
            failures.append(f"{label} {tid}: expected {expected}, got {item.get('status')}")
