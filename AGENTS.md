...
`STATUS.md` is authoritative. The synchronized 2026-09-12 frontier has one open leaf, `Hirsch.common_face_diameter_of_dim_ge_six` (`87a8b4f4-8b58-4340-8cb9-5fd1b548d01e`). Polynomial circuit-to-edge refinement remains an open ancestor. The deferred clipping/target-cone assembly and maximum-support regime are verified in merged PR #200. Integration #201 verifies exact selected-degree accounting, the selected-run excess sum, and unchanged #202 intrinsic carrier/fixed-deficit routes. Every fixed support deficit g is closed at cost D+2(g+3)2^g*r; an actual carrier has dimension h<=its excess. Merged #203 verifies positive projective graph transport and publishes the finite-certificate hidden-product n-d bound (b6289eea-78b3-4bcf-a5d7-65fbab46a986, Proved). Merged #204 records exact homogeneous chart discovery, a Proved recursive geometric-tree n-d theorem (4afd7668-51a9-4a5e-a991-2a02c53b9e1c), and locally verified D+n*r actual-pair clipping cost. Merged #205 now verifies the positive-feedback d-edge theorem and its actual-carrier adapter; public theorem 4096cd8a-6bf6-4788-89be-e67f84f6c92f is Proved. Merged #206 verifies all-available-row mass sum(delta)+r*s<=r*(n-d)+3s, an actual full-availability basis-star certificate, and dimension-capped linear routing; its public graph-budget theorem b8f45079-9784-43a0-b659-0914a2038332 is Proved. Verified integration #209 proves extraction of the d-row target basis, automatic full-availability mass, 1+3(n-d) mixed feedback/small-excess routes, and 1+24(n-d) feedback/dimension-five routes. Strict feasibility remains explicit; geometric assemblies have their own local/hosted verification, distinct from public theorem verdicts. Its final hosted gate passed and public basis theorem e46935cf-6286-4874-a742-01cc62269b2d is Proved; see STATUS.md for exact receipts. #207 is merged with its near-geodesic occurrence-budget theorem Proved. #208 now passes local and final hosted verification, including its continued cyclic transport/kernel modules; its conditional additive-spill proof is submitted and awaiting the platform verdict recorded in STATUS.md. Full geometric selection remains outstanding, and constant detour slack is ruled out within the specified first-edge/available-facet repair family. The residual target is arbitrary high-dimensional carriers not covered by these sufficient models and cross-level control of repeated row charges. A factor-three sibling mass sum alone does not prove a polynomial recurrence. Cubes rule out forcing logarithmic used-support deficit. Consult STATUS.md and the public-results inventory before reproving or republishing partial results.
...
The durable comment gate is `.github/workflows/prove2me-comment-publish.yml`. Once a locally green standalone packet is on a same-repo PR, post `/prove2me publish` as `jjoshua2`. The verify job compiles the PR SHA without the API key; only the trusted `main` publisher sees `PROVE2ME_API_KEY`. `/prove2me verify` compiles without publishing. `/prove2me verify --targets Module.Name` is a module-only Lake compile for intermediate theorem modules; publication still requires a packet. Packet preambles must not contain local `def`/`structure` declarations; a separate local compile of statement and solution can still WA on Prove2Me if those types are not identical. This does not replace local Lean iteration and is not a push-triggered experiment workflow.
