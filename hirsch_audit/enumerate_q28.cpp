// Exact enumeration of all feasible nonsingular 5-row bases of the posted Q28 polar.
// Build: g++ -O2 -std=c++17 enumerate_q28.cpp -o enumerate_q28
// Run: ./enumerate_q28 > vertices_raw.txt
...
int main(){
  ...
  for(int i0=0;i0<24;++i0) for(int i1=i0+1;i1<25;++i1)
  for(int i2=i1+1;i2<26;++i2) for(int i3=i2+1;i3<27;++i3)
  for(int i4=i3+1;i4<28;++i4){
    ...
    std::array<I,5> nums;
    for(int j=0;j<5;++j){Mat b=m;for(int i=0;i<5;++i)b[i][j]=1;nums[j]=det(b);}
    if(den<0){den=-den;for(auto &v:nums)v=-v;}
    bool ok=true;
    for(int i=0;i<28;++i){I s=0;for(int j=0;j<5;++j)s+=A[i][j]*nums[j];if(s>den){ok=false;break;}}
