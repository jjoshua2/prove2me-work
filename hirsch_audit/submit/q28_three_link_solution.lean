...
/-- Exhaustive chamber 5-row check: every ordered 5-basis is singular, infeasible,
or (if it has five original actives) one of the 20 sign-orbit representatives. -/
theorem verifyBasis_all :
    ∀ (s0 s1 s2 s3 s4 : Fin 14),
      s0 < s1 → s1 < s2 → s2 < s3 → s3 < s4 →
      verifyBasis s0 s1 s2 s3 s4 = true := by
  native_decide
...
def commonActiveCard (o1 o2 : Fin 20) (s : Fin 16) : ℕ :=
  (Finset.univ.filter (fun i : Fin 28 =>
    intDotFlip o1 0 i = orbitDen o1 ∧ intDotFlip o2 s i = orbitDen o2)).card
...
lemma extreme_nonneg_orbit {y : EuclideanSpace ℝ (Fin 5)}
  ...
  have hvb := verifyBasis_all s0 s1 s2 s3 s4 hs01 hs12 hs23 hs34
  have hexo : ∃ o : Fin 20, matchesOrbit nums d o = true := by
    have : verifyBasis s0 s1 s2 s3 s4 =
        decide (∃ o : Fin 20, matchesOrbit nums d o = true) := by
      dsimp [verifyBasis]
      have hne : (d = 0) = False := by simp [d, hdet5]
      simp [nums, d, hdet5, hfeas, Nat.not_lt.2 hge]
    exact of_decide_eq_true (by simpa [this] using hvb)
  obtain ⟨o, ho⟩ := hexo
  refine ⟨o, ?_⟩
  ext j
  have hmatch : ∀ k : Fin 5, orbitNum o k * d = nums k * orbitDen o :=
    of_decide_eq_true (by simpa [matchesOrbit] using ho)
  have hj := hmatch j
  have hden : (orbitDen o : ℝ) ≠ 0 := Int.cast_ne_zero.2 (orbitDen_ne_zero o)
  simp [orbitPoint_apply, hy_eq]
  field_simp [hden, hdR]
  norm_cast
  linarith [hj]
